...
DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_EFFECT, DP_TYPE_ENUM},
  {DPID_FLASH, DP_TYPE_ENUM},
  {DPID_ICON, DP_TYPE_ENUM},
  {DPID_MONTION, DP_TYPE_ENUM},
};
...
void all_data_update(void)
{
    ...
    mcu_dp_value_update(DPID_BRIGHT,当前亮度); //VALUE型数据上报;
    mcu_dp_string_update(DPID_COLOR,当前颜色指针,当前颜色数据长度); //STRING型数据上报;
    mcu_dp_enum_update(DPID_EFFECT,当前效果); //枚举型数据上报;
    mcu_dp_enum_update(DPID_FLASH,当前闪烁节奏); //枚举型数据上报;
    mcu_dp_enum_update(DPID_ICON,当前图案); //枚举型数据上报;
    mcu_dp_enum_update(DPID_MONTION,当前动画); //枚举型数据上报;
...
/*****************************************************************************
函数名称 : dp_download_bright_handle
功能描述 : 针对DPID_BRIGHT的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_bright_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long bright;
    
    bright = mcu_get_dp_download_value(value,length);
    /*
    //VALUE类型数据处理
    ...
    */
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_value_update(DPID_BRIGHT,bright);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_color_handle
功能描述 : 针对DPID_COLOR的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_color_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为STRING
    unsigned char ret;
    /*
    ...
    */
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_string_update(DPID_COLOR,value, length);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_effect_handle
功能描述 : 针对DPID_EFFECT的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_effect_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char effect;
    
    effect = mcu_get_dp_download_enum(value,length);
    switch(effect) {
        case 0:
        break;
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_enum_update(DPID_EFFECT, effect);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_flash_handle
功能描述 : 针对DPID_FLASH的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_flash_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char flash;
    
    flash = mcu_get_dp_download_enum(value,length);
    switch(flash) {
        case 0:
        break;
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_enum_update(DPID_FLASH, flash);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_icon_handle
功能描述 : 针对DPID_ICON的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_icon_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char icon;
    
    icon = mcu_get_dp_download_enum(value,length);
    switch(icon) {
        case 0:
        break;
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_enum_update(DPID_ICON, icon);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_montion_handle
功能描述 : 针对DPID_MONTION的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_montion_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char montion;
    
    montion = mcu_get_dp_download_enum(value,length);
    switch(montion) {
        case 0:
        break;
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_enum_update(DPID_MONTION, montion);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    ...
    switch(dpid) {
        ...
        case DPID_BRIGHT:
            //亮度处理函数
            ret = dp_download_bright_handle(value,length);
        break;
        case DPID_COLOR:
            //颜色处理函数
            ret = dp_download_color_handle(value,length);
        break;
        case DPID_EFFECT:
            //效果处理函数
            ret = dp_download_effect_handle(value,length);
        break;
        case DPID_FLASH:
            //闪烁节奏处理函数
            ret = dp_download_flash_handle(value,length);
        break;
        case DPID_ICON:
            //图案处理函数
            ret = dp_download_icon_handle(value,length);
        break;
        case DPID_MONTION:
            //动画处理函数
            ret = dp_download_montion_handle(value,length);
        break;
